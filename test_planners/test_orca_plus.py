...
from motion.orca_planner import ORCAPlanner
...
class ORCATest:
    def __init__(self, yaml_path: str):
        ...
        self.planner = ORCAPlanner(yaml_path)
        ...
        self.logger = MetricsLogger(
            algorithm="orca_plus",
            env_name=env_name,
            run_id=f"{env_name}_orca_plus",
            results_dir="results"
        )

        pg.init()
        self.screen = pg.display.set_mode((self.screen_w, self.screen_h))
        pg.display.set_caption("ORCA+ Planner")
    ...
    def _draw_obstacles(self):
        ...
        for obs in self.cfg.get("obstacles", []):
            if obs["type"] == "rectangle":
                ...
                x0, y0 = self.w2s(cx - w / 2.0, cy - h / 2.0)
                w_px = int(w * self.px_per_cm)
                h_px = int(h * self.px_per_cm)
    ...
    def _draw_agents(self):
        for agent in self.planner.agents:
            ...
            pg.draw.circle(
                ...
                agent.color,
                (sx, sy),
                int(agent.radius * self.px_per_cm),
                2
            )
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python test_orca_plus.py /ruta/al/env.yaml")
        sys.exit(1)

    yaml_path = sys.argv[1]
    test = ORCATest(yaml_path)
    test.run()
