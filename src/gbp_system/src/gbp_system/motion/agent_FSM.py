...
import numpy as np
from gbp_system.fg.gaussian import Gaussian
from gbp_system.fg.factor_graph import VNode, FNode, FactorGraph
    ...
    def step_move(self):
        ...
        if not other_agents:
            ...
            # --------------------------------------------------------
                ...
                # En FOLLOW_PATH no ponemos vx,vy = 0.
                # Calculamos la velocidad nominal entre puntos consecutivos del path,
                # para que PyBullet pueda orientar el robot con atan2(vy, vx).
                for i in range(self._steps):
                    ...
                    px, py = path[path_index][:2]

                    if path_index < len(path) - 1:
                        nx, ny = path[path_index + 1][:2]
                        vx = (nx - px) / self._dt
                        vy = (ny - py) / self._dt
                    else:
                        vx = 0.0
                    ...
                    s = np.array([px, py, vx, vy]).reshape(4, 1)
                ...
                # El prior de inicio queda fijado al primer punto del horizonte.
                self._fnode_start._factor = Gaussian(
                    self._vnodes[0].dims,
                    self._vnodes[0].belief.mean.copy(),
                    high_conf_cov.copy(),
                )
